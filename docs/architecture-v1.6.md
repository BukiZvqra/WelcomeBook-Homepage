# WelcomeBook Agency — Архитектура v1.6
...
```json
{
  ...
  "bnbforms_account_id": "belora-residence",
  "bnbforms_enabled": true,
  "booking_widget_position": "hero"
}
```
...
### Стъпка 4.5: BNBForms setup (1-2 часа, виж секция 8.6)
- Клиент се регистрира на bnbforms.com (free trial)
- Configure имоти, цени, наличности
- Свързваш iCal с Booking + Airbnb
- Subscribe ($16/мес директно на клиента)
- Запис на `bnbforms_account_id` в `config.json`
- Test резервация → проверка на sync

> **Note:** За първите 2-3 клиенти отнема 2-3 часа. След 5-ия клиент → 1 час workflow.
...
## 8.6. BOOKING SYSTEM INTEGRATION — BNBForms (DEFAULT за всички клиенти)

> **Decisive решение, не опция:** Всеки клиентски уебсайт ще включва **BNBForms** като booking engine + channel manager. Не build-ваме custom booking system.

### Защо BNBForms (не custom build)

| Критерий | Custom build | BNBForms | Решение |
|----------|--------------|----------|---------|
| Setup време per клиент | 60-100 часа | 1-2 часа | ✅ BNBForms |
| Maintenance burden | Постоянен | Нулев | ✅ BNBForms |
| PCI compliance | Огромен legal риск | Тяхно проблем | ✅ BNBForms |
| iCal sync с Booking/Airbnb | Build от 0 | Out-of-box | ✅ BNBForms |
| Payment processing | Stripe integration manually | Stripe/PayPal/etc out-of-box | ✅ BNBForms |
| Скейлируемост (50 клиента) | Невъзможно за 1 човек | Тривиална | ✅ BNBForms |
| Cost per клиент | $0 (но много часове) | $16/мес (клиентът плаща) | ✅ BNBForms |

**Решение:** BNBForms е **default за всички клиенти**. Custom build е забранен освен в edge cases (Premium tier клиент с €5000+ budget).

### Какво е BNBForms

- **Embeddable booking widget** за всеки сайт (HTML, WordPress, Wix, Webflow, etc.)
- **PMS** (Property Management System) — клиентът сам управлява имоти, цени, наличности
- **iCal channel manager** — синхронизация с Booking.com, Airbnb, TripAdvisor
- **Payment processing** — Stripe, PayPal, bank transfer, pay on arrival
- **Цена:** $16/мес unlimited properties + bookings, **0% commission**
- **Free trial:** 30 дни
- **На пазара:** 7+ години (proven, не startup)

### Как се интегрира в нашите сайтове

**Стъпка 1:** Клиентът (или ти от негово име) се регистрира на bnbforms.com

**Стъпка 2:** В BNBForms admin:
- Добавяш всички имоти на клиента
- Настройваш цени, правила, минимален стой
- Свързваш iCal с Booking, Airbnb (чрез export/import URLs)
- Configure payment method (Stripe препоръчителен)

**Стъпка 3:** BNBForms генерира unique widget code:
```html
<div id="bnbforms-widget" data-account="yavor-tes-borovets"></div>
<script src="https://bnbforms.com/widget.js" async></script>
```

**Стъпка 4:** В Eleventy template, embedваме widget-а на правилните места:
```nunjucks
{# templates/website-multiproperty/src/_includes/booking-widget.njk #}
<section class="booking-widget" id="book">
  <h2>{{ client.business_name }} — Резервация</h2>
  <div id="bnbforms-widget" data-account="{{ client.bnbforms_account_id }}"></div>
</section>

<script src="https://bnbforms.com/widget.js" async></script>
```

**Стъпка 5:** В `clients/[id]/website/config.json` добавяме:
```json
{
  "bnbforms_account_id": "yavor-tes-borovets",
  "bnbforms_enabled": true,
  "booking_widget_position": "hero" 
}
```

### Workflow за нов клиент (с BNBForms)

```
1. Клиент подписва договор + плаща setup такса (теб)         ← ти
2. Клиент купува домейн                                       ← клиент
3. Клиент се регистрира на BNBForms.com (free trial)          ← клиент или ти
4. Subscribe-ва $16/мес ($16 директно на BNBForms)            ← клиент
5. Configure имоти в BNBForms admin (1-2 часа)                ← ти + клиент
6. Свържи iCal с Booking + Airbnb                             ← ти
7. Test реална резервация                                     ← ти
8. Build уебсайта в Eleventy (template + config)              ← ти + Claude Code
9. Embed BNBForms widget в template                           ← ти
10. Deploy на Netlify                                          ← ти
11. DNS setup                                                  ← клиент
12. Production deploy                                          ← ти
13. Клиент получава първа онлайн резервация → ✅              ← work
```

### Pricing implications (информативно)

> **Note:** Тази секция е заради architecture-pricing alignment. Реалните цени живеят в Master Playbook v3.

| Услуга | Кой плаща | Сума |
|--------|-----------|------|
| Custom website build (Eleventy) | Клиент → Теб | €399-€799 setup |
| Welcome Book Portal + AI chatbot | Клиент → Теб | €120 setup + €5/мес |
| BNBForms subscription | Клиент → BNBForms | $16/мес |
| Cloudinary | Теб → Cloudinary | €0 (free tier) |
| Netlify hosting | Теб → Netlify | €0 (free tier) |

**Архитектурна последица:** Setup work на нов клиент е **3-4 часа повече** (BNBForms onboarding) спрямо план без booking. Pricing tiers трябва да отразяват това.

### iCal sync — как работи в реалност
...
└──────────────┘                    │   sync       │
                                    ...
                                    └──┬────────▲──┘
                                       │        │
                              outbound │        │ inbound
                              iCal feed│        │ direct booking
                              (avail.) │        │ (от widget на сайта)
                                       ▼        │
                                  ┌────────────────┐
                                  │ Booking Widget │
                                  │ embedded на    │
...
Двупосочен flow:
- Booking.com + Airbnb изпращат iCal feeds → BNBForms знае кога нещо е резервирано там
- Direct резервации идват ОТ widget-а ВЪВ BNBForms (не през iCal)
- BNBForms exports iCal обратно към Booking + Airbnb → те знаят кога имотът е резервиран директно
```

**Важно за клиентите:**
- Sync не е real-time — има 5-15 мин delay
- Това може да причини **rare double-bookings** при много high-volume properties
- BNBForms има protection срещу double-bookings (auto-blocking на dates)
- За критични properties → клиентът може да upgrade към премиум plan на BNBForms

### Едно-нещо, което винаги обясняваме на клиентите

> "BNBForms работи на subscription с тях директно ($16/мес), не през нас. Ние пускаме widget-а на сайта и помагаме с настройка, но subscription-ът и резервациите минават през BNBForms. Парите идват **директно на вас** — ние нямаме achess до тях."

Това е **критично** legal-wise:
- ❌ Никога не debit-вай BNBForms subscription за клиента (PCI risk)
- ❌ Никога не пипаш guest payment data
- ✅ Винаги клиентът = primary BNBForms account holder
- ✅ Ти си collaborator (read-only) ако клиентът сподели достъп

### Алтернативи (за extreme edge cases)

В 95% от случаите → BNBForms. Но за:

| Use case | Алтернатива |
|----------|-------------|
| Клиент с 50+ имота, premium budget | **Hostaway** (€100+/мес, full PMS) |
| Клиент който вече има Smoobu | Запази Smoobu, embed-ваме техния widget |
| Клиент който иска нативна resa без 3rd party | **Lodgify** (€15-€60/мес, по-нативна интеграция) |

> **PRO TIP:** Не предлагаме алтернативи първоначално. BNBForms е default. Ако клиентът explicitly иска друго → обсъждаме custom approach.
...
| **BNBForms** | €0 (клиентът плаща) | Клиентът плаща директно $16/мес на BNBForms. Ти не пипаш парите. |
...
- **BNBForms upgrade tier** — кога да препоръчаме премиум plan на клиента?
...
`WelcomeBook_Architecture_Reference_v1.pdf` визуализира секции 1-6 в print-ready формат с brand colors (#0A0F1E + #E8820C). **PDF v1.0 е значително outdated** спрямо този markdown — отразява само v1.0 структурата. Следващата редакция (PDF v2.0) трябва да отразява всички промени през v1.1 → v1.6:
...
**От v1.2:**
- Build System · Eleventy (секция 6)
- Chatbot Knowledge Loading (секция 10)
- Multi-language strategy (секция 13)

**От v1.3:**
- Conversation State (секция 11)
- Tone presets (секция 12)

**От v1.4:**
- Netlify Deploy Configuration (секция 8.5) — site IDs mapping, DNS records, deploy workflow

**От v1.5:**
- Booking System Integration · BNBForms (секция 8.6) — booking engine + iCal channel manager
- Updated config.json schema (bnbforms_account_id, bnbforms_enabled, booking_widget_position)
- Updated Cost Structure (BNBForms ред)
- Updated Setup Workflow (Стъпка 4.5: BNBForms onboarding)

**От v1.6:**
- iCal sync diagram correction (двупосочен flow)
- Widget script tag fix (само async, не async+defer)

**Image Storage** (секция 15) и **Chatbot Resilience** (секция 16) са semi-stable от v1.2.

> **PRO TIP:** Не build-вай нов PDF преди да имаш първи завършен Yavor build. Реалното използване на архитектурата ще разкрие пропуски, които ще искаш да са в PDF v2.0.

---

## Changelog

### v1.5 → v1.6 (peer review fixes — small but important)

**3 fixes от code review:**

1. **Widget script tag fix** — премахнат конфликт между `async` и `defer` в Eleventy template примера. `async` и `defer` заедно са безсмислени (`async` игнорира `defer`). Сега само `async`. Малка грешка, но ще се copy-paste-не в реален код.

2. **iCal sync diagram correction** — старата диаграма показваше "Direct sites (Yavor) → iCal export → BNBForms", което е концептуално грешно. Direct резервации идват **ВЪВ** BNBForms през widget-а, не през iCal. Новата диаграма показва двупосочен flow: (a) Booking + Airbnb изпращат iCal feeds към BNBForms (inbound), (b) BNBForms exports iCal обратно към тях (outbound), (c) direct резервации идват от widget-а на сайта.

3. **Companion PDF reference обновен** — старата бележка изброяваше само v1.2-v1.3 промени и беше озаглавена "PDF v1.3". Сега изброява всички промени през v1.1 → v1.6, organized по версия. Когато правим PDF v2.0, ще има пълен checklist какво да включи.

**Архитектурно следствие:** Нула. Това са документационни fixes — не променят build flow, config schema, или workflow.

### v1.4 → v1.5 (Booking integration architecture)

**Architectural decision:** BNBForms е default booking engine за всички клиенти. Custom booking system е забранен освен в Premium edge cases.

**Промени:**

1. **НОВА СЕКЦИЯ 8.6: Booking System Integration — BNBForms** — пълен план за integration:
   - Сравнителна таблица custom build vs BNBForms (BNBForms wins по 7 критерия)
   - 5-стъпков integration workflow в Eleventy templates
   - 13-стъпков workflow за нов клиент (включително BNBForms onboarding)
   - iCal sync diagram
   - Pricing implications (информативно)
   - Legal/PCI правила (клиентът е primary BNBForms holder, не теб)
   - Алтернативни системи (Hostaway, Smoobu, Lodgify) — само за edge cases

2. **Setup workflow обновен (секция 21)** — добавена Стъпка 4.5: BNBForms setup (1-2 часа per нов клиент).

3. **config.json пример обновен (секция 7)** — добавени `bnbforms_account_id`, `bnbforms_enabled`, `booking_widget_position` полета.

4. **Cost Structure обновена (секция 20)** — добавен ред за BNBForms ($16/мес, плаща се от клиента, не теб).

5. **Future Questions обновени (секция 24)** — премахнат "Booking widget" въпрос (вече решен), добавен "BNBForms upgrade tier" въпрос.

**Архитектурно следствие:** Setup time per нов клиент: +3-4 часа за BNBForms onboarding. Pricing tiers трябва да отразят това (Master Playbook v3 update).

**Какво НЕ се променя:** Eleventy build процес, Cloudinary integration, chatbot architecture, portal templates. BNBForms е "drop-in" widget който добавяме в template без структурни промени.
